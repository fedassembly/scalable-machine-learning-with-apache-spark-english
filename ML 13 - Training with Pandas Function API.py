...
def train_model(df_pandas: pd.DataFrame) -> pd.DataFrame:
    ...
    with mlflow.start_run(run_id=run_id) as outer_run:
        ...
        with mlflow.start_run(
            run_name=str(device_id), nested=True, experiment_id=experiment_id
        ) as run:
            ...
            return_df = pd.DataFrame(
                [[device_id, n_used, artifact_uri, mse]],
...
with mlflow.start_run(run_name="Training session for all devices") as run:
    ...
    model_directories_df = (
        df.withColumn("run_id", f.lit(run_id))  # Add run_id
        .groupby("device_id")
...
prediction_df = combined_df.groupby("device_id").applyInPandas(
    apply_model, schema=apply_return_schema
)
...
model_df = (
    spark.read.format("mlflow-experiment")
    .load(experiment_id)
...
device_to_model = {
    row["device"]: mlflow.sklearn.load_model(f"runs:/{row['run_id']}/{row['device']}")
...
example_model.predict(None, combined_df.toPandas().head(20))
